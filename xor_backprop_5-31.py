...
def error(theOut,expOut,nnWeights):
    # print(theOut)
    # print([expOut[i]*nnWeights[8] for i in range(len(expOut))])
    # print(sum([.5 * (theOut[i] - expOut[i]*nnWeights[8]) for i in range(len(expOut))]))
    return [.5 * (theOut[i] - expOut[i]*nnWeights[8]) for i in range(len(expOut))]
...
for x in range(100):
    ...
    for j in range(len(nnWeights)):
        NEWnnW = nnWeights[:]
        NEWnnW[j] += step*random.random()
        # nnWeights[j] + step*random.random()
        expOut = []
        ...
        # print('expOut',expOut)
        nerrorlist = error(theOut,expOut,NEWnnW)
        errorlist = error(theOut,expOut,nnWeights)
        if abs(sum(nerrorlist))<abs(sum(errorlist)):
            # errorlist = nerrorlist
            # print('here')
            nnWeights = NEWnnW
    ...
    ##     nnWeights[i] = nnWeights[i]+gradient[i]
